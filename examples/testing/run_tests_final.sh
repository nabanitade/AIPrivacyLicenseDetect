...
# Function to run Python test with timeout protection
run_python_test() {
    local test_name="$1"
    local python_code="$2"
    ...
    output=$(timeout 10s python3 -c "$python_code" 2>&1)
    exit_code=$?
    
    if [ $exit_code -eq 124 ]; then
        print_status "FAIL" "$test_name (timeout after 10s)"
        return 1
        ...
        if echo "$output" | grep -q "$expected_pattern"; then
            ...
            print_status "FAIL" "$test_name (pattern not found)"
            return 1
        fi
    else
        print_status "FAIL" "$test_name (Python error: $exit_code)"
        return 1
    fi
}
