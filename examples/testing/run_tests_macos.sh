...
# macOS-compatible version (no timeout command)
...
# Function to run Python test with simple error handling
run_python_test() {
    local test_name="$1"
    local python_code="$2"
    ...
    # Simple approach: run the command and check exit code
    if python3 -c "$python_code" 2>/dev/null | grep -q "$expected_pattern"; then
