...
main() {
    ...
    if python3 test_local_detection.py >/dev/null 2>&1; then
        print_status "PASS" "Comprehensive test script completed"
    else
    ...
    if [[ $FAILED_TESTS -eq 0 ]]; then
        ...
        echo
        echo -e "${GREEN}Your AI Privacy License Detection Library is working perfectly!${NC}"
        echo -e "${GREEN}All detection methods, parsing, and CLI features are functional.${NC}"
        exit 0
    else
        print_status "ERROR" "Some tests failed. Please check the output above."
        echo
        echo -e "${YELLOW}Failed tests indicate issues that need to be resolved.${NC}"
        echo -e "${YELLOW}Check the error messages and fix any problems.${NC}"
        exit 1
    fi
}
...
if ! command -v bc &> /dev/null; then
    echo -e "${YELLOW}Warning: 'bc' command not found. Performance timing may not work correctly.${NC}"
    echo -e "${YELLOW}Install bc for accurate performance measurements.${NC}"
    echo
fi
